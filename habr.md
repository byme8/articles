...
# Введение
 ...
 Думаю первое что стоить уточнить это то, что понимается под "маленькой рефлексией". Я предлагаю реализовать для всех типов метод ``` Dictionary<string, Type> GetBakedType() ```, который будет возвращать имена пропертей и их тип. Поскольку мне хотелось чтобы это работало со всеми типами, то самым простым вариантом будет генерация метода расширения(extention method) для каждого типа. Ручная его реализация будет иметь следующий вид:
 ``` cs
...
```
 ...
 Хочу предупредить данная реализация не рассчитана на реальное применение. Я лишь хочу показать способ как можно организовать кодогенерацию.
...
И первым ключевым моментом станет то, что если у нас есть настоящая ошибка компиляции, то анализаторы не запускаются вовсе. Как результат, если мы попытаемся вызвать наш ``` GetBakedType() ``` в контексте типа для которого он не реализован, то получим ошибку компиляции и все наши старания не будут иметь смысла. Но тут нам поможет знание о том с каким приоритетом компилятор вызывает методы расширения. Весь сок в том, что конкретные реализации имеют больший приоритет чем универсальные методы(generic). То есть в следующем примере будет вызван второй метод, а не первый:
...
Кодогенерацию мы будем делать через ``` CodeFixProvider ```, который подписан на наш анализатор. В первую очередь нам необходимо проверить что получилось найти нашему анализатору. Это никак не влияет на поток выполнения, а лишь определяет какой текст будет в меню при его открытии. 
...
В результате у нас получился Roslyn анализатор-кодогенератор при помощи которого  реализовуется "маленькая" рефлексия с использованием кодогенерации. Будет сложно придумать реальное применение текущей реализации, но она будет прекрасным примером для реализации легко доступных кодогенераторов. Данный подход может быть, как и любая кодогенерация, полезен при реализации сериализаторов. Моя тестовая реализация сериализатора MessagePack-а работала на ~20% быстрее чем [neuecc/MessagePack-CSharp](https://github.com/neuecc/MessagePack-CSharp), а более быстрого сериализатора я пока не находил. Кроме того моя реализация не требует ``` Roslyn.Emit ```, что прекрасно подходит для Unity и AOT сценариях.